...
contract PoIClaimProcessor is Ownable {
    ...
    event PayoutAuthorized(bytes32 indexed paymentId, address indexed submitter);
    ...
    function processNonArrivalProof(
        ...
        require(publishedRoots[targetShardRoot], "PoI: Target root not published");
        require(!isPayoutAuthorized[paymentId], "PoI: Payout already authorized");
        ...
        require(value.length == 0, "PoI: Key was found in the trie (inclusion proof provided)");
     ...
     */
    function getProofSubmitter(bytes32 paymentId) external view returns (address) {
