...
contract CollateralVault is ICollateralVault, ReentrancyGuard, Ownable {
    ...
    modifier onlySettlement() {
        require(isSettlementContract[msg.sender], "CollateralVault: Caller is not an authorized settlement contract");
        _;
    }
    ...
    constructor(address _srt, address _usdc) {
        require(_srt != address(0), "CollateralVault: SRT address cannot be zero");
        require(_usdc != address(0), "CollateralVault: USDC address cannot be zero");
        srt = IERC20(_srt);
        usdc = IERC20(_usdc);
    }
    ...
    function addSettlementContract(address _contractAddress) external onlyOwner {
        require(_contractAddress != address(0), "CollateralVault: Contract address cannot be zero");
        require(!isSettlementContract[_contractAddress], "CollateralVault: Contract already authorized");
    ...
    function removeSettlementContract(address _contractAddress) external onlyOwner {
        require(isSettlementContract[_contractAddress], "CollateralVault: Contract not authorized");
    ...
    function depositSRT(uint256 amount) external nonReentrant {
        require(amount > 0, "CollateralVault: Amount must be greater than zero");
    ...
    function withdrawSRT(uint256 amount) external nonReentrant {
        ...
        require(currentStake >= amount, "CollateralVault: Insufficient free SRT stake");
    ...
    function withdrawUSDC(uint256 amount) external nonReentrant {
        ...
        require(currentStake >= amount, "CollateralVault: Insufficient free USDC stake");
    ...
    function lockSRT(address user, uint256 amount) external override nonReentrant onlySettlement {
        require(amount > 0, "CollateralVault: Amount must be greater than zero");
        uint256 userStake = srtStake[user];
        require(userStake >= amount, "CollateralVault: Insufficient free SRT to lock");
        srtStake[user] = userStake - amount;
        srtLocked[user] += amount;
        emit LockedSRT(user, amount);
    }

    function releaseSRT(address user, uint256 amount) external override nonReentrant onlySettlement {
        require(amount > 0, "CollateralVault: Amount must be greater than zero");
        uint256 userLocked = srtLocked[user];
        require(userLocked >= amount, "CollateralVault: Insufficient locked SRT to release");
        srtLocked[user] = userLocked - amount;
        srtStake[user] += amount;
        emit ReleasedSRT(user, amount);
    }

    function slashSRT(address user, uint256 amount, address recipient) external override nonReentrant onlySettlement {
        require(amount > 0, "CollateralVault: Amount must be greater than zero");
        require(recipient != address(0), "CollateralVault: Recipient address cannot be zero");
        uint256 userLocked = srtLocked[user];
        require(userLocked >= amount, "CollateralVault: Insufficient locked SRT to slash");
        srtLocked[user] = userLocked - amount;
        srt.safeTransfer(recipient, amount);
        emit SlashedSRT(user, recipient, amount);
    }

    function lockUSDC(address user, uint256 amount) external override nonReentrant onlySettlement {
        require(amount > 0, "CollateralVault: Amount must be greater than zero");
        uint256 userStake = usdcStake[user];
        require(userStake >= amount, "CollateralVault: Insufficient free USDC to lock");
        usdcStake[user] = userStake - amount;
        usdcLocked[user] += amount;
        emit LockedUSDC(user, amount);
    }

    function releaseUSDC(address user, uint256 amount) external override nonReentrant onlySettlement {
        require(amount > 0, "CollateralVault: Amount must be greater than zero");
        uint256 userLocked = usdcLocked[user];
        require(userLocked >= amount, "CollateralVault: Insufficient locked USDC to release");
        usdcLocked[user] = userLocked - amount;
        usdcStake[user] += amount;
        emit ReleasedUSDC(user, amount);
    }

    function slashUSDC(address user, uint256 amount, address recipient) external override nonReentrant onlySettlement {
        require(amount > 0, "CollateralVault: Amount must be greater than zero");
        require(recipient != address(0), "CollateralVault: Recipient address cannot be zero");
        uint256 userLocked = usdcLocked[user];
        require(userLocked >= amount, "CollateralVault: Insufficient locked USDC to slash");
        usdcLocked[user] = userLocked - amount;
